...
$Script = Join-Path $Root "code\03_data_processing\88B_anchor_robustness_and_paired_contrasts.R"

if (-not (Test-Path $Script)) {
    Write-Host "ERROR: Step88B R script not found:"
    Write-Host $Script
    exit 1
}
...
if ($null -eq $selected) {
    ...
    Write-Host 'source("E:/sepsis_project/code/03_data_processing/88B_anchor_robustness_and_paired_contrasts.R", echo=TRUE)'
    exit 1
}
