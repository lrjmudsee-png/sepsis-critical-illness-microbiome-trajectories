...
$Script = Join-Path $Root "code\03_data_processing\89B_taxonomic_robustness_and_provenance.R"

if (-not (Test-Path $Script)) {
    Write-Host "ERROR: Step89B R script not found:"
    Write-Host $Script
    exit 1
}
...
if ($null -eq $selected) {
    ...
    Write-Host 'source("E:/sepsis_project/code/03_data_processing/89B_taxonomic_robustness_and_provenance.R", echo=TRUE)'
    exit 1
}
