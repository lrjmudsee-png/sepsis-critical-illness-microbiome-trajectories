# ============================================================
# Sepsis V2 - Step 88A2
# FIXED: cohort-specific longitudinal diversity + within-patient
...
LOG <- file.path(
  OUT,
  "_STEP88A2_runtime_checkpoints.txt"
)

ERR <- file.path(
  OUT,
  "_STEP88A2_FATAL_ERROR.txt"
)
...
resolve_time_labels <- function(d) {
  tr <- clean_chr(d$time_raw)
  tk <- clean_chr(d$timepoint_key)
  ...
  # A source label must not silently represent multiple exact time orders.
  # Example found in PRJNA516701:
  #   "middle" -> DAY_3 and DAY_5
  #   "late"   -> DAY_7 and DAY_10
  # If such a collision occurs, prefer exact timepoint_key labels for
  # the whole cohort where available.
  map <- tibble(
    label = label,
    ord = as.numeric(d$analysis_time_order)
  ) |>
    filter(
      !is.na(label),
      !is.na(ord)
...
bind_rows_cohort_safe <- function(x) {
  if (!length(x)) {
    return(tibble())
  }
...
main <- function() {
  ...
  for (proj in project_order) {
    ...
    if (nrow(long) > 0) {
      ...
      long$time_order_numeric <- as.numeric(
        long$analysis_time_order
      )

      long$time_label <- resolve_time_labels(
        long
      )
      ...
      # Bray displacement inference:
      # Baseline/reference rows are exactly zero by construction, so they
      # are retained for descriptive trajectories and plots but excluded
      # from inferential mixed models. Otherwise a baseline-vs-follow-up
      # test would be structurally biased toward significance.
      displacement_followup <- displacement |>
        filter(
          !is_patient_reference
        )

      b_factor <- safe_lmer(
        d = displacement_followup,
        outcome =
          "bray_from_patient_baseline",
        time_mode = "factor",
        include_depth = FALSE,
      ...
      if (
        nrow(displacement_followup) > 0 &&
        all(
          !is.na(
            displacement_followup$time_day
          )
        ) &&
        n_distinct(
          displacement_followup$time_day
  ...
  alpha_all <- bind_rows_cohort_safe(
    alpha_rows
  )

  alpha_summary_all <- bind_rows_cohort_safe(
    alpha_summary_rows
  )

  displacement_all <- bind_rows_cohort_safe(
    displacement_rows
  )

  displacement_summary_all <- bind_rows_cohort_safe(
    displacement_summary_rows
  )

  pcoa_all <- bind_rows_cohort_safe(
    pcoa_rows
  )

  model_global <- bind_rows_cohort_safe(
    model_global_rows
  )

  model_coef <- bind_rows_cohort_safe(
    model_coef_rows
  )

  model_status <- bind_rows_cohort_safe(
    model_status_rows
  )
  ...
  if (
    ...
    stop(
      "Final Step88A2 primary-state guard failed."
    )
  }
  ...
  if (
    ...
    stop(
      paste0(
        "Final Step88A2 longitudinal-state guard failed. ",
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_ALPHA,
      "V2_STEP88A2_ALL_sample_alpha_diversity.csv"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_ALPHA,
      "V2_STEP88A2_ALL_longitudinal_alpha_summary.csv"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_BETA,
      "V2_STEP88A2_ALL_within_patient_bray_displacement_summary.csv"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_PCOA,
      "V2_STEP88A2_ALL_Bray_PCoA_coordinates.csv"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_MODEL,
      "V2_STEP88A2_model_fixed_effect_coefficients.csv"
    )
  )
  ...
  readme <- c(
    "SEPSIS V2 - STEP88A2 LONGITUDINAL DIVERSITY AND DISPLACEMENT",
    paste0(
      "Created: ",
      Sys.time()
    ),
    ...
    "Baseline/reference Bray distances are exactly zero by construction and are descriptive only; inferential Bray models use follow-up observations only.",
    ...
    "Step88A2 models are cohort-specific trajectory tests. They are not yet cross-cohort meta-analysis.",
    "Do not interpret intervention-support cohorts as natural-history sepsis replication cohorts.",
    "",
    paste0(
      "Model fits with non-OK status: ",
      n_model_fail,
      ". Check 05_MODELS/V2_STEP88A2_model_status.csv."
    ),
    "",
    "NEXT",
    "After auditing Step88A2 results, proceed to taxonomic longitudinal trajectories and then cross-cohort direction/effect synthesis."
  )
  ...
  completion_status <- if (
    ...
    "STEP88A2 COMPLETE WITH MODEL WARNINGS"
  }
