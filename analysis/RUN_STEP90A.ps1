...
$Script = Join-Path $Root "code\03_data_processing\90A_ecological_robustness_and_shared_taxonomy.R"

if (-not (Test-Path $Script)) {
    Write-Host "ERROR: Step90A R script not found:"
    Write-Host $Script
    exit 1
}
...
if ($null -eq $selected) {
    ...
    Write-Host 'source("E:/sepsis_project/code/03_data_processing/90A_ecological_robustness_and_shared_taxonomy.R", echo=TRUE)'
    exit 1
}
