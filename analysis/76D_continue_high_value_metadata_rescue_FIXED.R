...
START_LOG <- file.path(
  OUT_ROOT,
  "_STEP76D_runtime_checkpoints.txt"
)
...
write_excel_csv(
  ...
  file.path(
    OUT_ROOT,
    "V2_step76D_final_status.csv"
  ),
  na=""
)

cat(
  paste0("STEP76D COMPLETE: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n"),
  file=START_LOG,
  append=TRUE
)
...
cat("\n============================================================\n")
cat("SEPSIS V2 - STEP 76D COMPLETE\n")
