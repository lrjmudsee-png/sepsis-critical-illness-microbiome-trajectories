...
LOG <- file.path(
  OUT,
  "_STEP85A_runtime_checkpoints.txt"
)

ERR <- file.path(
  OUT,
  "_STEP85A_FATAL_ERROR.txt"
)
...
verify_reference_md5 <- function() {
  ...
  if (
    ...
    !file.exists(MD5_PATH)
  ) {
    stop(
      "Training FASTA or MD5 file missing."
    )
  }
  ...
  if (!length(lines)) {
    stop(
      "SILVA MD5 file is empty."
    )
  }

  expected <- str_extract(
    lines[1],
    regex("[A-Fa-f0-9]{32}")
  )
  ...
  if (
    is.na(expected) ||
    is.na(observed) ||
    tolower(expected)!=tolower(observed)
  ) {
    stop(
      paste0(
        "SILVA training-set MD5 verification failed.\n",
        "Expected: ",expected,"\n",
        "Observed: ",observed
      )
    )
  ...
  tibble(
    file=TRAIN_FILE,
    expected_md5=tolower(expected),
    observed_md5=tolower(observed),
    md5_match=TRUE
  )
}
...
main <- function() {
  ...
  cat("\n============================================================\n")
  cat("SEPSIS V2 - STEP85A COMPLETE\n")
  cat("============================================================\n\n")
