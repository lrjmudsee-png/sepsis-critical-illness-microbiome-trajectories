...
OUT_ROOT <- file.path(
  PROJECT_ROOT,
  "results",
  "V2_16C_high_value_metadata_rescue_fixed"
)
...
# Immediate checkpoint: if this exists, the script truly started.
START_LOG <- file.path(
  OUT_ROOT,
  "_STEP76C_runtime_checkpoints.txt"
)
...
cat(
  paste0("PRJEB68229 WIDE FIXED: ", format(Sys.time(), "%H:%M:%S"), "\n"),
  file=START_LOG,
  append=TRUE
)
...
cat(
  paste0("PRJEB68229 QC COMPLETE: ", format(Sys.time(), "%H:%M:%S"), "\n"),
  file=START_LOG,
  append=TRUE
)
...
cat(
  paste0("PRJNA1125274 COMPLETE: ", format(Sys.time(), "%H:%M:%S"), "\n"),
  file=START_LOG,
  append=TRUE
)
...
profile851_df <- if (
  ...
  tibble(
    ...
    patient_field=logical(),
    sample_field=logical(),
    time_field=logical(),
    infection_field=logical()
  )
}
...
cra_profiles_df <- if (
  ...
  tibble(
    ...
    sample_field=logical(),
    time_field=logical(),
    sepsis_field=logical(),
    mortality_field=logical()
  )
}
...
write_excel_csv(
  ...
  file.path(
    OUT_ROOT,
    "V2_step76C_final_status.csv"
  ),
  na=""
)

cat(
  paste0("STEP76C COMPLETE: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n"),
  file=START_LOG,
  append=TRUE
)
...
cat("\n============================================================\n")
cat("SEPSIS V2 - STEP 76C COMPLETE\n")
