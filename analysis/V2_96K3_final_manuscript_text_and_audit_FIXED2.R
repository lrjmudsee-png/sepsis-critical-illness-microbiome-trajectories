...
OUT <- file.path(
  RESULTS,
  "V2_96K3_FINAL_MANUSCRIPT_TEXT_AND_AUDIT_FIXED2"
)
...
# Explicit manuscript-facing clarification:
# common-anchor is a sensitivity analysis, not independent replication.
if(!str_detect(
  txt,
  fixed("The common-anchor analysis was treated as a sensitivity analysis rather than an independent replication.")
)){
  anchor_sentence <- "The common-anchor analysis served as a sensitivity analysis rather than an independent replication."

  if(str_detect(txt,fixed(anchor_sentence))){
    txt <- str_replace(
      txt,
      fixed(anchor_sentence),
      "The common-anchor analysis was treated as a sensitivity analysis rather than an independent replication."
    )
  } else {
    # If the exact prior sentence differs, append the clarification to the
    # cross-cohort reproducibility paragraph without changing any statistics.
    cross_anchor <- "The common-anchor analysis served as a sensitivity analysis"

    if(str_detect(txt,fixed(cross_anchor))){
      txt <- str_replace(
        txt,
        fixed(cross_anchor),
        "The common-anchor analysis was treated as a sensitivity analysis rather than an independent replication"
...
guardrails <- tibble(
  ...
  evidence_present=c(
    ...
    str_detect(txt,regex("common-anchor.*sensitivity analysis.*rather than an independent replication",ignore_case=TRUE)),
...
MANUSCRIPT_OUT <- file.path(
  OUT,
  "Manuscript_v5_2_SUBMISSION_DRAFT_FIXED2.txt"
)
...
# Step96J3 retained the exact hash-verified supplementary source assets
# under _SOURCE_* names. Prefer manuscript-facing names if present, but
# fall back to those exact verified source files.
supp_pdf_candidates <- c(
  file.path(
    FIG3_FINAL_DIR,
...
pick_existing <- function(paths,label){
  ...
  if(length(hit)<1){
    stop(
      "Missing Step96J3 supplementary asset (",
      label,
      "). Tried: ",
...
supp_pdf <- pick_existing(
  supp_pdf_candidates,
  "PDF"
)

supp_png <- pick_existing(
  supp_png_candidates,
  "PNG"
)

supp_legend <- pick_existing(
  supp_legend_candidates,
  "LEGEND"
)
...
writeLines(
  c(
    "V2 STEP96K3 FINAL MANUSCRIPT TEXT AND AUDIT — SUPPLEMENTARY ASSET FIX",
    "",
    paste0("Title: ",TITLE),
    "",
    paste0(
