...
$Script = Join-Path $Root "code\03_data_processing\89A2_taxonomic_paired_trajectory_screen_FIXED.R"

if (-not (Test-Path $Script)) {
    Write-Host "ERROR: Step89A2 fixed R script not found:"
    Write-Host $Script
    exit 1
}
...
if ($null -eq $selected) {
    ...
    Write-Host 'source("E:/sepsis_project/code/03_data_processing/89A2_taxonomic_paired_trajectory_screen_FIXED.R", echo=TRUE)'
    exit 1
}
