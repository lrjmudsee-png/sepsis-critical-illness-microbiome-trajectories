# ============================================================
# Sepsis V2 - Step 89A
# Paired taxonomic trajectory screen (Genus + Family)
#
# Scientific design:
...
OUT <- file.path(
  ROOT,
  "results",
  "V2_29A_STEP89A_TAXONOMIC_PAIRED_TRAJECTORIES"
)
...
LOG <- file.path(
  OUT,
  "_STEP89A_runtime_checkpoints.txt"
)

ERR <- file.path(
  OUT,
  "_STEP89A_FATAL_ERROR.txt"
)
...
main <- function() {
  ...
  if (
    ...
    stop(
      "Step89A QC row-count guard failed."
    )
  }
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_QC,
      "V2_STEP89A_taxonomic_analysis_QC.csv"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_GENUS,
      "V2_STEP89A_ALL_genus_paired_CLR_results.csv"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_FAMILY,
      "V2_STEP89A_ALL_family_paired_CLR_results.csv"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_SYN,
      "V2_STEP89A_PRIMARY_common_anchor_all_taxa.csv"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_SYN,
      "V2_STEP89A_GENUS_cross_cohort_trajectory_synthesis.csv"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_SYN,
      "V2_STEP89A_FAMILY_cross_cohort_trajectory_synthesis.csv"
    )
  )
  ...
  make_effect_heatmap(
    ...
    file.path(
      DIR_PLOT,
      "V2_STEP89A_GENUS_effect_size_heatmap.pdf"
    )
  )
  ...
  make_effect_heatmap(
    ...
    file.path(
      DIR_PLOT,
      "V2_STEP89A_FAMILY_effect_size_heatmap.pdf"
    )
  )
  ...
  write_csv_safe(
    ...
    file.path(
      DIR_SYN,
      "V2_STEP89A_CORE_SEPSIS_taxonomic_shortlist.csv"
    )
  )
  ...
  readme <- c(
    "SEPSIS V2 - STEP89A TAXONOMIC PAIRED TRAJECTORIES",
    paste0(
      "Created: ",
      Sys.time()
    ),
    ...
    "",
    "NEXT",
    "Audit Step89A candidates. If coherent, proceed to focused longitudinal models/visualization for prioritized taxa and manuscript-level cross-cohort evidence synthesis."
  )
