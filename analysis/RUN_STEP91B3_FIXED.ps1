...
$Script = Join-Path $Root "code\03_data_processing\91B3_infection_source_freeze_and_analysis_FIELD_COLLISION_FIXED.R"

if (-not (Test-Path $Script)) {
    Write-Host "ERROR: Step91B3 R script not found:"
    Write-Host $Script
    exit 1
}
...
if ($null -eq $selected) {
    ...
    Write-Host 'source("E:/sepsis_project/code/03_data_processing/91B3_infection_source_freeze_and_analysis_FIELD_COLLISION_FIXED.R", echo=TRUE)'
    exit 1
}
