...
$Script = Join-Path $Root "code\03_data_processing\91A_infection_source_feasibility_audit.R"

if (-not (Test-Path $Script)) {
    Write-Host "ERROR: Step91A R script not found:"
    Write-Host $Script
    exit 1
}
...
if ($null -eq $selected) {
    ...
    Write-Host 'source("E:/sepsis_project/code/03_data_processing/91A_infection_source_feasibility_audit.R", echo=TRUE)'
    exit 1
}
