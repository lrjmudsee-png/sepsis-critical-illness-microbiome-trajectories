...
$Script = Join-Path $Root "code\03_data_processing\90B_corrected_coherence_inference.R"

if (-not (Test-Path $Script)) {
    Write-Host "ERROR: Step90B R script not found:"
    Write-Host $Script
    exit 1
}
...
if ($null -eq $selected) {
    ...
    Write-Host 'source("E:/sepsis_project/code/03_data_processing/90B_corrected_coherence_inference.R", echo=TRUE)'
    exit 1
}
