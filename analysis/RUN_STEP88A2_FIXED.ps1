...
$Script = Join-Path $Root "code\03_data_processing\88A2_longitudinal_diversity_and_displacement_FIXED.R"

if (-not (Test-Path $Script)) {
    Write-Host "ERROR: fixed Step88A2 R script not found:"
    Write-Host $Script
    exit 1
}
...
if ($null -eq $selected) {
    ...
    Write-Host 'source("E:/sepsis_project/code/03_data_processing/88A2_longitudinal_diversity_and_displacement_FIXED.R", echo=TRUE)'
    exit 1
}
